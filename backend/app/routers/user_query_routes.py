...
from app.util import extract_alert_info, fetchAlerts
...
async def submit_question(request: QuestionsRequest):
    global current_task
    ...
    current_task = asyncio.create_task(process_query(request))
...
# Async function
async def process_query(request: QuestionsRequest):
    facts = []
    zip_alerts = []
    async with lock:
        try:
            ...
            try:
                ...
                zip_alerts = await fetchAlerts(user['zip'])
            ...
            # Fetch GPT response
            gpt_response = await generate_response(query=request.question, alerts=extracted_info, user_info=str(user))

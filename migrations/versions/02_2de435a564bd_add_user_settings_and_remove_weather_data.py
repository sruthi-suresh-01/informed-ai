...
def upgrade() -> None:
    ...
    # Add empty settings for admin users
    op.execute(
        """
        INSERT INTO settings (id, user_id, created_at, updated_at, configurations)
        SELECT
            gen_random_uuid(),
            users.user_id,
            NOW(),
        ...
        AND NOT EXISTS (
            SELECT 1 FROM settings WHERE settings.user_id = users.user_id
        );
        """
    )
